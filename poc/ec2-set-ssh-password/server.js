...
try { db.exec(`ALTER TABLE sessions ADD COLUMN pem_content TEXT`); } catch (_) {}
try { db.exec(`ALTER TABLE sessions ADD COLUMN health_status TEXT DEFAULT 'unknown'`); } catch (_) {}
try { db.exec(`ALTER TABLE sessions ADD COLUMN last_checked TEXT`); } catch (_) {}
try { db.exec(`ALTER TABLE sessions ADD COLUMN name TEXT DEFAULT ''`); } catch (_) {}
try { db.exec(`ALTER TABLE sessions ADD COLUMN os_detected TEXT DEFAULT ''`); } catch (_) {}
try { db.exec(`ALTER TABLE sessions ADD COLUMN last_connected TEXT`); } catch (_) {}
try { db.exec(`ALTER TABLE sessions ADD COLUMN connection_result TEXT DEFAULT ''`); } catch (_) {}
db.exec(`CREATE INDEX IF NOT EXISTS idx_sessions_host ON sessions(host)`);
db.exec(`CREATE INDEX IF NOT EXISTS idx_sessions_ts ON sessions(timestamp DESC)`);
...
const stmtInsertSnippet = db.prepare(`INSERT INTO command_snippets (name, command, category, host) VALUES (@name, @command, @category, @host)`);
const stmtGetSnippets = db.prepare(`SELECT * FROM command_snippets ORDER BY category, name`);
const stmtDeleteSnippet = db.prepare(`DELETE FROM command_snippets WHERE id = @id`);

const stmtInsertRecording = db.prepare(`INSERT INTO terminal_recordings (name, host, data, duration) VALUES (@name, @host, @data, @duration)`);
const stmtGetRecordings = db.prepare(`SELECT id, name, host, duration, created_at FROM terminal_recordings ORDER BY created_at DESC`);
const stmtGetRecording = db.prepare(`SELECT * FROM terminal_recordings WHERE id = @id`);
const stmtDeleteRecording = db.prepare(`DELETE FROM terminal_recordings WHERE id = @id`);
...
const stmtUpsertSession = db.prepare(`
    ...
    WHERE id = @id
`);
const stmtGetSessionPem = db.prepare(`SELECT pem_content, pem_filename FROM sessions WHERE id = @id`);
...
const stmtUpdateHealth = db.prepare(`UPDATE sessions SET health_status = @status, last_checked = @ts WHERE host = @host`);
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
        ...
        switch (msg.type) {
            ...
            case 'docker-logs-start': {
                ...
                sess.conn.exec(`docker logs -f --tail 50 ${safeLogId} 2>&1`, (err, logStream) => {
                    if (err) return;
                    dockerLogStreams.set(logKey, logStream);
                    logStream.on('data', (data) => {
                        const c = clients.get(msg.clientId);
                        if (c && c.readyState === WebSocket.OPEN) {
                            c.send(JSON.stringify({ type: 'docker-log-output', containerId: msg.containerId, data: data.toString('utf-8') }));
                        }
                    });
                    logStream.on('close', () => { dockerLogStreams.delete(logKey); });
                });
                break;
            }
            ...
            case 'docker-shell-start': {
                ...
                sess2.conn.exec(`docker exec -it ${safeShellId} /bin/sh -c "command -v bash > /dev/null 2>&1 && exec bash || exec sh"`, {
                    pty: { term: 'xterm-256color', cols: msg.cols || 120, rows: msg.rows || 30 }
                }, (err, shellStream) => {
                    if (err) {
                        const c = clients.get(msg.clientId);
                        if (c && c.readyState === WebSocket.OPEN) {
                            c.send(JSON.stringify({ type: 'docker-shell-output', data: '\r\nFailed to attach: ' + err.message + '\r\n' }));
                        }
                        return;
                    }
                    dockerShellStreams.set(msg.clientId, { stream: shellStream, containerId: msg.containerId });
                    shellStream.on('data', (data) => {
                        const c = clients.get(msg.clientId);
                        if (c && c.readyState === WebSocket.OPEN) {
                            c.send(JSON.stringify({ type: 'docker-shell-output', data: data.toString('utf-8') }));
                        }
                    });
            ...
            case 'docker-shell-resize': {
                ...
                if (ds2 && ds2.stream && msg.cols && msg.rows) {
                    try { ds2.stream.setWindow(msg.rows, msg.cols, 0, 0); } catch (_) {}
                }
                break;
            }
            ...
            case 'docker-shell-stop': {
                ...
                if (ds3 && ds3.stream) { ds3.stream.destroy(); dockerShellStreams.delete(msg.clientId); }
                break;
            }
            case 'cloud-init-start': {
                ...
                sess.conn.exec('tail -f /var/log/cloud-init-output.log 2>/dev/null', (err, stream) => {
                    if (err) return;
                    cloudInitStreams.set(msg.clientId, stream);
                    stream.on('data', (data) => {
                        const c = clients.get(msg.clientId);
                        if (c && c.readyState === WebSocket.OPEN) {
                            c.send(JSON.stringify({ type: 'cloud-init-output', data: data.toString('utf-8') }));
                        }
                    });
                    stream.on('close', () => { cloudInitStreams.delete(msg.clientId); });
                });
                break;
            }
    ...
    ws.on('close', () => {
        ...
        if (cloudInitStreams.has(clientId)) {
            try { cloudInitStreams.get(clientId).destroy(); } catch (_) {}
...
function execOnTerminal(clientId, command) {
    return new Promise((resolve, reject) => {
        const session = terminals.get(clientId);
        if (!session || !session.conn) return reject(new Error('No active SSH session'));
...
async function detectDistro(clientId) {
    ...
    const output = await execOnTerminal(clientId,
    ...
    else if (['amzn', 'centos', 'rhel', 'fedora', 'ol', 'rocky', 'almalinux'].includes(id)) {
        try { await execOnTerminal(clientId, 'command -v dnf'); pkgManager = 'dnf'; }
        catch (_) { pkgManager = 'yum'; }
    }
...
const VALID_TIMEZONES = [
    ...
    'Asia/Kolkata', 'Asia/Mumbai', 'Asia/Tokyo', 'Asia/Shanghai', 'Asia/Hong_Kong',
    'Asia/Singapore', 'Asia/Seoul', 'Asia/Dubai', 'Asia/Bangkok', 'Asia/Jakarta',
    'Asia/Taipei', 'Asia/Karachi', 'Asia/Dhaka', 'Asia/Riyadh',
...
// Upload PEM key
app.post('/api/upload-key', requireAuth, upload.single('pemKey'), (req, res) => {
    if (!req.file) {
...
app.post('/api/setup-password', requireAuth, async (req, res) => {
    const { password, username, pemKeyPath, ec2Host, clientId, useUserData } = req.body;
...
app.post('/api/sessions', requireAuth, (req, res) => {
    try {
        const { host, username, mode, auth_method, pem_filename, pem_content, status, name } = req.body;
        ...
        if (existing) {
            ...
            if (!uniqueHosts.has(hostTrimmed) && uniqueHosts.size >= 5) {
                return res.status(400).json({ error: 'Maximum 5 hosts reached. Delete a session first.' });
            }
...
app.delete('/api/sessions/:id', requireAuth, (req, res) => {
    try {
        ...
        if (isNaN(numId)) return res.status(400).json({ error: 'Invalid session ID' });
...
app.get('/api/sessions/:id/pem', requireAuth, (req, res) => {
    try {
        ...
        if (!row || !row.pem_content) {
            return res.status(404).json({ error: 'No PEM key saved for this session' });
        }
...
app.post('/api/recordings', requireAuth, (req, res) => {
    try {
        ...
        const result = stmtInsertRecording.run({
            name, host: host || '', data, duration: duration || 0
        });
...
app.delete('/api/recordings/:id', requireAuth, (req, res) => {
    try {
        ...
        logAudit('recording_delete', `Deleted recording #${req.params.id}`, '', req);
        res.json({ success: true });
    } catch (err) {
...
app.post('/api/processes', requireAuth, async (req, res) => {
    ...
    try {
        const output = await execOnTerminal(clientId, 'ps aux --sort=-%cpu | head -51');
...
app.post('/api/processes/kill', requireAuth, async (req, res) => {
    ...
    if (!clientId || !pid) return res.status(400).json({ error: 'clientId and pid required' });
    try {
...
const COMMON_PORT_LABELS = {
    ...
    6379: 'Redis', 27017: 'MongoDB', 8080: 'App', 8443: 'HTTPS-Alt',
    3000: 'Dev', 5000: 'Flask', 8000: 'Alt-HTTP', 9090: 'Prometheus',
    2379: 'etcd', 6443: 'K8s-API', 25: 'SMTP', 53: 'DNS', 111: 'RPC',
    993: 'IMAPS', 995: 'POP3S', 1433: 'MSSQL', 11211: 'Memcached'
};
...
function parsePortsOutput(output) {
    ...
    for (const line of lines) {
        if (line.startsWith('State') || line.startsWith('Netid') || line.startsWith('Proto') || !line.trim()) continue;
        ...
        let port = 0, address = '', program = '', state = '';
...
function parseServicesOutput(output) {
    ...
    for (const line of lines) {
        ...
        const status = parts[2] === 'active' ? 'running' : (parts[2] === 'failed' ? 'failed' : parts[2]);
...
app.post('/api/services', requireAuth, async (req, res) => {
    ...
    try {
        const [portsOut, servicesOut] = await Promise.all([
            execOnTerminal(clientId, 'ss -tlnp 2>/dev/null || netstat -tlnp 2>/dev/null').catch(() => ''),
            execOnTerminal(clientId, 'systemctl list-units --type=service --state=running,failed --no-pager --no-legend 2>/dev/null').catch(() => '')
        ]);
...
app.post('/api/network', requireAuth, async (req, res) => {
    ...
    try {
        const [connectionsOut, interfacesOut] = await Promise.all([
            execOnTerminal(clientId, 'ss -tupn 2>/dev/null || netstat -tupn 2>/dev/null').catch(() => ''),
...
function parsePasswd(passwdOut, sudoOut, wheelOut) {
    ...
    for (const line of [sudoOut, wheelOut]) {
        ...
        if (parts.length >= 4) {
            parts[3].split(',').filter(Boolean).forEach(u => sudoUsers.add(u));
        }
    }
    // root is always sudo
    sudoUsers.add('root');
...
app.post('/api/users/add', requireAuth, async (req, res) => {
    ...
    if (!clientId || !username || !password) return res.status(400).json({ error: 'clientId, username, and password required' });
    ...
    try {
        ...
        await execOnTerminal(clientId, `echo '${sanitizePath(username)}:${sanitizePath(password)}' | chpasswd`);
        if (sudo) {
            await execOnTerminal(clientId, `usermod -aG sudo '${sanitizePath(username)}' 2>/dev/null || usermod -aG wheel '${sanitizePath(username)}' 2>/dev/null`).catch(() => {});
        }
        logAudit('user_add', `Added user: ${username}${sudo ? ' (sudo)' : ''}`, '', req);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/users/delete', requireAuth, async (req, res) => {
    ...
    if (username === 'root') return res.status(400).json({ error: 'Cannot delete root user' });
    try {
        await execOnTerminal(clientId, `userdel -r '${sanitizePath(username)}' 2>/dev/null || userdel '${sanitizePath(username)}'`);
...
app.post('/api/users/sudo', requireAuth, async (req, res) => {
    ...
    try {
        if (enable) {
            await execOnTerminal(clientId, `usermod -aG sudo '${sanitizePath(username)}' 2>/dev/null || usermod -aG wheel '${sanitizePath(username)}' 2>/dev/null`);
        } else {
            await execOnTerminal(clientId, `gpasswd -d '${sanitizePath(username)}' sudo 2>/dev/null; gpasswd -d '${sanitizePath(username)}' wheel 2>/dev/null`).catch(() => {});
        }
        logAudit('user_sudo_toggle', `${enable ? 'Enabled' : 'Disabled'} sudo for: ${username}`, '', req);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/ssh-keys', requireAuth, async (req, res) => {
    ...
    if (!clientId) return res.status(400).json({ error: 'clientId required' });
    try {
        const output = await execOnTerminal(clientId, 'cat ~/.ssh/authorized_keys 2>/dev/null').catch(() => '');
...
app.post('/api/ssh-keys/add', requireAuth, async (req, res) => {
    ...
    if (!clientId || !publicKey) return res.status(400).json({ error: 'clientId and publicKey required' });
    ...
    try {
        ...
        await execOnTerminal(clientId, `echo '${safeKey}' >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys`);
...
app.post('/api/ssh-keys/delete', requireAuth, async (req, res) => {
    ...
    if (!clientId || lineIndex === undefined) return res.status(400).json({ error: 'clientId and lineIndex required' });
    try {
        const lineNum = parseInt(lineIndex) + 1;
        await execOnTerminal(clientId, `sed -i '${lineNum}d' ~/.ssh/authorized_keys`);
...
app.post('/api/docker/check', requireAuth, async (req, res) => {
    ...
    if (!clientId) return res.status(400).json({ error: 'clientId required' });
    try {
        const output = await execOnTerminal(clientId, 'docker version --format "{{.Server.Version}}" 2>/dev/null');
        const dockerVersion = output.trim().split('\n')[0] || '';
        let composeVersion = '';
        try {
            const cv = await execOnTerminal(clientId, 'docker compose version --short 2>/dev/null || docker-compose --version 2>/dev/null');
            composeVersion = cv.trim().split('\n')[0] || '';
        } catch (_) {}
        res.json({ success: true, installed: !!dockerVersion, dockerVersion, composeVersion, hasCompose: !!composeVersion });
    } catch (err) {
        res.json({ success: true, installed: false, dockerVersion: '', composeVersion: '', hasCompose: false });
    }
});
...
app.post('/api/docker/containers', requireAuth, async (req, res) => {
    ...
    try {
        const [containersOut, statsOut] = await Promise.all([
            execOnTerminal(clientId, "docker ps -a --format '{{.ID}}|{{.Names}}|{{.Image}}|{{.Status}}|{{.Ports}}|{{.State}}|{{.CreatedAt}}|{{.Size}}'"),
            execOnTerminal(clientId, "docker stats --no-stream --format '{{.Container}}|{{.Name}}|{{.CPUPerc}}|{{.MemUsage}}|{{.MemPerc}}|{{.NetIO}}|{{.BlockIO}}|{{.PIDs}}'").catch(() => '')
        ]);
        const containers = parseDockerContainers(containersOut);
        const stats = parseDockerStats(statsOut);
        const statsMap = new Map();
        for (const s of stats) { statsMap.set(s.containerId, s); statsMap.set(s.name, s); }
        for (const c of containers) {
            const s = statsMap.get(c.id) || statsMap.get(c.name);
            if (s) { c.cpu = s.cpuPercent; c.mem = s.memPercent; c.memUsage = s.memUsage; c.netIO = s.netIO; c.blockIO = s.blockIO; c.pids = s.pids; }
        }
...
app.post('/api/docker/container/action', requireAuth, async (req, res) => {
    ...
    if (!clientId || !containerId || !action) return res.status(400).json({ error: 'clientId, containerId, and action required' });
    const validActions = ['start', 'stop', 'restart', 'pause', 'unpause', 'remove'];
    if (!validActions.includes(action)) return res.status(400).json({ error: 'Invalid action' });
    const safeId = sanitizePath(containerId);
    const cmd = action === 'remove' ? `docker rm --force ${safeId}` : `docker ${action} ${safeId}`;
    try {
...
app.post('/api/docker/container/inspect', requireAuth, async (req, res) => {
    ...
    if (!clientId || !containerId) return res.status(400).json({ error: 'clientId and containerId required' });
    try {
        const output = await execOnTerminal(clientId, `docker inspect ${sanitizePath(containerId)} 2>/dev/null`);
...
app.post('/api/docker/container/health', requireAuth, async (req, res) => {
    ...
    if (!clientId || !containerId) return res.status(400).json({ error: 'clientId and containerId required' });
    try {
        const output = await execOnTerminal(clientId, `docker inspect --format '{{json .State.Health}}' ${sanitizePath(containerId)} 2>/dev/null`);
...
app.post('/api/docker/container/env', requireAuth, async (req, res) => {
    ...
    if (!clientId || !containerIds || !containerIds.length) return res.status(400).json({ error: 'clientId and containerIds required' });
    try {
        const envResults = {};
        for (const id of containerIds.slice(0, 10)) {
            const output = await execOnTerminal(clientId, `docker inspect --format '{{json .Config.Env}}' ${sanitizePath(id)} 2>/dev/null`);
...
app.post('/api/docker/container/logs', requireAuth, async (req, res) => {
    ...
    const safeId = sanitizePath(containerId);
    try {
        const output = await execOnTerminal(clientId, `docker logs --tail ${safeTail} --timestamps ${safeId} 2>&1`);
...
app.post('/api/docker/images', requireAuth, async (req, res) => {
    ...
    if (!clientId) return res.status(400).json({ error: 'clientId required' });
    try {
        const output = await execOnTerminal(clientId, "docker images --format '{{.Repository}}|{{.Tag}}|{{.ID}}|{{.CreatedSince}}|{{.Size}}'");
...
app.post('/api/docker/images/pull', requireAuth, async (req, res) => {
    ...
    if (!clientId || !imageName) return res.status(400).json({ error: 'clientId and imageName required' });
    try {
        const output = await execOnTerminal(clientId, `docker pull ${sanitizePath(imageName)} 2>&1`);
...
app.post('/api/docker/images/remove', requireAuth, async (req, res) => {
    ...
    if (!clientId || !imageId) return res.status(400).json({ error: 'clientId and imageId required' });
    try {
...
app.post('/api/docker/images/layers', requireAuth, async (req, res) => {
    ...
    if (!clientId || !imageId) return res.status(400).json({ error: 'clientId and imageId required' });
    try {
        const output = await execOnTerminal(clientId, `docker history --no-trunc --format '{{.CreatedBy}}|{{.Size}}|{{.CreatedAt}}' ${sanitizePath(imageId)} 2>/dev/null`);
        const layers = output.trim().split('\n').filter(Boolean).map(line => {
            const parts = line.split('|');
            return { createdBy: parts[0] || '', size: parts[1] || '', createdAt: parts[2] || '' };
        });
...
app.post('/api/docker/compose/detect', requireAuth, async (req, res) => {
    ...
    if (!clientId) return res.status(400).json({ error: 'clientId required' });
    try {
        const output = await execOnTerminal(clientId, "find / -maxdepth 4 \\( -name 'docker-compose.yml' -o -name 'docker-compose.yaml' -o -name 'compose.yml' -o -name 'compose.yaml' \\) 2>/dev/null | head -20");
...
app.post('/api/docker/compose/status', requireAuth, async (req, res) => {
    ...
    if (!clientId || !composePath) return res.status(400).json({ error: 'clientId and composePath required' });
    try {
        const dir = sanitizePath(require('path').dirname(composePath));
        const output = await execOnTerminal(clientId, `cd '${dir}' && docker compose ps --format '{{.Name}}|{{.Service}}|{{.Status}}|{{.Ports}}|{{.State}}' 2>/dev/null`);
...
app.post('/api/docker/compose/action', requireAuth, async (req, res) => {
    ...
    if (!clientId || !composePath || !action) return res.status(400).json({ error: 'clientId, composePath, and action required' });
    const validActions = ['up -d', 'down', 'restart', 'pull'];
    if (!validActions.includes(action)) return res.status(400).json({ error: 'Invalid action' });
    try {
        const dir = sanitizePath(require('path').dirname(composePath));
        const output = await execOnTerminal(clientId, `cd '${dir}' && docker compose ${action} 2>&1`);
        logAudit('docker_compose_' + action.split(' ')[0], `Compose ${action}: ${composePath}`, '', req);
        res.json({ success: true, output });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/docker/system/prune', requireAuth, async (req, res) => {
    ...
    try {
        const output = await execOnTerminal(clientId, `docker system prune --force${allFlag} 2>&1`);
...
app.post('/api/docker/system/df', requireAuth, async (req, res) => {
    ...
    if (!clientId) return res.status(400).json({ error: 'clientId required' });
    try {
        const output = await execOnTerminal(clientId, 'docker system df 2>/dev/null');
...
app.post('/api/docker/volumes', requireAuth, async (req, res) => {
    ...
    if (!clientId) return res.status(400).json({ error: 'clientId required' });
    try {
        const output = await execOnTerminal(clientId, "docker volume ls --format '{{.Name}}|{{.Driver}}|{{.Scope}}|{{.Mountpoint}}'");
...
app.post('/api/docker/volumes/inspect', requireAuth, async (req, res) => {
    ...
    if (!clientId || !volumeName) return res.status(400).json({ error: 'clientId and volumeName required' });
    try {
        const output = await execOnTerminal(clientId, `docker volume inspect ${sanitizePath(volumeName)} 2>/dev/null`);
        const inspect = JSON.parse(output);
        res.json({ success: true, inspect: Array.isArray(inspect) ? inspect[0] : inspect });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.post('/api/docker/volumes/create', requireAuth, async (req, res) => {
    ...
    try {
        await execOnTerminal(clientId, `docker volume create${driverFlag} ${sanitizePath(name)}`);
...
app.post('/api/docker/volumes/remove', requireAuth, async (req, res) => {
    ...
    if (!clientId || !volumeName) return res.status(400).json({ error: 'clientId and volumeName required' });
    try {
        await execOnTerminal(clientId, `docker volume rm ${sanitizePath(volumeName)}`);
...
app.post('/api/docker/networks', requireAuth, async (req, res) => {
    ...
    if (!clientId) return res.status(400).json({ error: 'clientId required' });
    try {
        const output = await execOnTerminal(clientId, "docker network ls --format '{{.ID}}|{{.Name}}|{{.Driver}}|{{.Scope}}'");
...
app.post('/api/docker/networks/inspect', requireAuth, async (req, res) => {
    ...
    if (!clientId || !networkId) return res.status(400).json({ error: 'clientId and networkId required' });
    try {
        const output = await execOnTerminal(clientId, `docker network inspect ${sanitizePath(networkId)} 2>/dev/null`);
        const inspect = JSON.parse(output);
        res.json({ success: true, inspect: Array.isArray(inspect) ? inspect[0] : inspect });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/docker/networks/remove', requireAuth, async (req, res) => {
    ...
    if (!clientId || !networkId) return res.status(400).json({ error: 'clientId and networkId required' });
    try {
        await execOnTerminal(clientId, `docker network rm ${sanitizePath(networkId)}`);
...
app.post('/api/docker/networks/connect', requireAuth, async (req, res) => {
    ...
    if (!clientId || !networkId || !containerId) return res.status(400).json({ error: 'clientId, networkId, and containerId required' });
    try {
        await execOnTerminal(clientId, `docker network connect ${sanitizePath(networkId)} ${sanitizePath(containerId)}`);
        logAudit('docker_network_connect', `Connected ${containerId} to network ${networkId}`, '', req);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/docker/networks/disconnect', requireAuth, async (req, res) => {
    ...
    if (!clientId || !networkId || !containerId) return res.status(400).json({ error: 'clientId, networkId, and containerId required' });
    try {
        await execOnTerminal(clientId, `docker network disconnect ${sanitizePath(networkId)} ${sanitizePath(containerId)}`);
        logAudit('docker_network_disconnect', `Disconnected ${containerId} from network ${networkId}`, '', req);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
const sftpUpload = multer({
    storage: multer.diskStorage({
        ...
        filename: (req, file, cb) => cb(null, `sftp-${uuidv4()}-${file.originalname}`)
    }),
...
app.post('/api/sftp/list', requireAuth, async (req, res) => {
    ...
    if (!clientId) return res.status(400).json({ error: 'clientId required' });
    const session = terminals.get(clientId);
    if (!session || !session.conn) return res.status(400).json({ error: 'No active SSH session' });

    try {
        const sftp = await getSftp(session.conn);
        sftp.readdir(remotePath || '/home', (err, list) => {
            if (err) return res.status(500).json({ error: `Failed to list: ${err.message}` });
...
app.post('/api/sftp/upload', requireAuth, sftpUpload.single('file'), async (req, res) => {
    const { clientId, remotePath } = req.body;
    if (!clientId || !req.file) return res.status(400).json({ error: 'clientId and file required' });
    const session = terminals.get(clientId);
    if (!session || !session.conn) return res.status(400).json({ error: 'No active SSH session' });
    ...
    try {
        ...
        sftp.fastPut(localPath, remoteFile, {
            ...
            logAudit('file_upload', `Uploaded ${req.file.originalname} to ${remoteFile}`, '', req);
...
app.post('/api/sftp/download', requireAuth, async (req, res) => {
    const { clientId, remotePath } = req.body;
    if (!clientId || !remotePath) return res.status(400).json({ error: 'clientId and remotePath required' });
    const session = terminals.get(clientId);
    if (!session || !session.conn) return res.status(400).json({ error: 'No active SSH session' });
...
app.post('/api/sftp/read', requireAuth, async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, content: content.toString('utf-8'), size: stats.size, path: remotePath });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.post('/api/sftp/write', requireAuth, async (req, res) => {
    ...
    try {
        ...
        logAudit('file_edit', `Edited ${remotePath} (${buf.length} bytes)`, '', req);
...
app.post('/api/sftp/rename', requireAuth, async (req, res) => {
    ...
    if (!clientId || !oldPath || !newPath) return res.status(400).json({ error: 'clientId, oldPath, and newPath required' });
    const session = terminals.get(clientId);
    if (!session || !session.conn) return res.status(400).json({ error: 'No active SSH session' });
...
app.post('/api/sftp/chmod', requireAuth, async (req, res) => {
    ...
    if (!clientId || !remotePath || !permissions) return res.status(400).json({ error: 'clientId, remotePath, and permissions required' });
    if (!/^[0-7]{3,4}$/.test(permissions)) return res.status(400).json({ error: 'Invalid permissions format (use octal e.g. 755)' });
    const session = terminals.get(clientId);
    if (!session || !session.conn) return res.status(400).json({ error: 'No active SSH session' });
...
app.post('/api/sftp/chown', requireAuth, async (req, res) => {
    ...
    if (!clientId || !remotePath || !owner) return res.status(400).json({ error: 'clientId, remotePath, and owner required' });
    if (!/^[a-zA-Z0-9_-]+$/.test(owner)) return res.status(400).json({ error: 'Invalid owner name' });
    if (group && !/^[a-zA-Z0-9_-]+$/.test(group)) return res.status(400).json({ error: 'Invalid group name' });
    const session = terminals.get(clientId);
    if (!session || !session.conn) return res.status(400).json({ error: 'No active SSH session' });
...
app.post('/api/sftp/search', requireAuth, async (req, res) => {
    const { clientId, path: searchPath, query, searchContent, maxResults } = req.body;
    if (!clientId || !query) return res.status(400).json({ error: 'clientId and query required' });
    const session = terminals.get(clientId);
    if (!session || !session.conn) return res.status(400).json({ error: 'No active SSH session' });
...
app.post('/api/sftp/copy', requireAuth, async (req, res) => {
    ...
    if (!clientId || !srcPath || !destPath) return res.status(400).json({ error: 'clientId, srcPath, and destPath required' });
    const session = terminals.get(clientId);
    if (!session || !session.conn) return res.status(400).json({ error: 'No active SSH session' });
...
app.post('/api/resources', requireAuth, async (req, res) => {
    ...
    const cmd = 'uname -srm; uptime; free -b 2>/dev/null || vm_stat 2>/dev/null; df -B1 / 2>/dev/null || df -k / 2>/dev/null; cat /proc/cpuinfo 2>/dev/null | grep "model name" | head -1; nproc 2>/dev/null || sysctl -n hw.ncpu 2>/dev/null';
    ...
    conn.on('ready', () => {
        ...
        conn.exec(cmd, (err, stream) => {
            if (err) {
                conn.end();
                return res.status(500).json({ error: 'Failed to execute resource commands' });
            }
            ...
            stream.on('close', () => {
                ...
                try {
                    ...
                    res.json({ success: true, resources: { raw: output, parseError: true } });
                }
            });
        });
    });
    ...
    if (pemKeyPath && fs.existsSync(pemKeyPath)) {
        try { connectConfig.privateKey = fs.readFileSync(pemKeyPath); } catch (_) {}
    }
    if (password) {
...
function parseResourceOutput(output) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('load average')) {
            const loadMatch = line.match(/load average[s]?:\s*([\d.]+)[,\s]+([\d.]+)[,\s]+([\d.]+)/);
            if (loadMatch) {
                result.loadAvg = [parseFloat(loadMatch[1]), parseFloat(loadMatch[2]), parseFloat(loadMatch[3])];
            }
...
app.post('/api/system-info', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const interfaces = (sections[0] || '').split('\n')
...
app.post('/api/system/set-hostname', requireAuth, async (req, res) => {
    ...
    if (!clientId || !hostname) return res.status(400).json({ error: 'clientId and hostname required' });
    if (!/^[a-zA-Z0-9.-]+$/.test(hostname) || hostname.length > 64) {
        return res.status(400).json({ error: 'Invalid hostname (alphanumeric, hyphens, dots, max 64 chars)' });
    }
    try {
        await execOnTerminal(clientId, `hostnamectl set-hostname '${sanitizePath(hostname)}'`);
...
app.post('/api/system/set-timezone', requireAuth, async (req, res) => {
    ...
    if (!clientId || !timezone) return res.status(400).json({ error: 'clientId and timezone required' });
    if (!VALID_TIMEZONES.includes(timezone)) {
        return res.status(400).json({ error: 'Invalid timezone' });
    }
    try {
        await execOnTerminal(clientId, `timedatectl set-timezone '${sanitizePath(timezone)}'`);
...
app.post('/api/system/swap-status', requireAuth, async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, swapDevices, swapTotal, swapUsed, hasSwap: swapTotal > 0 });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.post('/api/system/swap-create', requireAuth, async (req, res) => {
    ...
    if (!validSizes.includes(sizeMB)) {
        return res.status(400).json({ error: 'Invalid swap size. Use: 256, 512, 1024, 2048, or 4096 MB' });
    }
    try {
        const steps = [
            { cmd: `fallocate -l ${sizeMB}M /swapfile 2>/dev/null || dd if=/dev/zero of=/swapfile bs=1M count=${sizeMB}`, msg: `Creating ${sizeMB}MB swap file...` },
            ...
            { cmd: 'grep -q "/swapfile" /etc/fstab || echo "/swapfile none swap sw 0 0" >> /etc/fstab', msg: 'Making swap persistent...' }
        ];
        ...
        for (const step of steps) {
            ...
            try {
                ...
                sendLog(clientId, { type: 'swap-progress', message: '  FAILED: ' + err.message, error: true });
...
app.post('/api/packages/list', requireAuth, async (req, res) => {
    ...
    try {
        ...
        let cmd;
        if (pkgManager === 'apt') {
            cmd = "dpkg-query -W -f='${Package}|${Version}|${db:Status-Status}|${Installed-Size}\\n' 2>/dev/null | head -500";
        } else {
            cmd = "rpm -qa --queryformat '%{NAME}|%{VERSION}-%{RELEASE}|installed|%{SIZE}\\n' 2>/dev/null | sort | head -500";
        }
        const output = await execOnTerminal(clientId, cmd);
        const packages = output.trim().split('\n').filter(l => l.includes('|')).map(line => {
            const [name, version, status, size] = line.split('|');
            return { name, version, status: status || 'installed', size: parseInt(size) || 0 };
        });
...
app.post('/api/packages/search', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const results = output.trim().split('\n').filter(l => l.trim()).map(line => {
            ...
            return { name: (parts[0] || '').trim().split(/\s/)[0], description: (parts[1] || '').trim() };
        }).filter(r => r.name);
...
app.post('/api/packages/info', requireAuth, async (req, res) => {
    ...
    if (!clientId || !packageName) return res.status(400).json({ error: 'clientId and packageName required' });
    if (!/^[a-zA-Z0-9._+:-]+$/.test(packageName)) return res.status(400).json({ error: 'Invalid package name' });
    try {
        const { pkgManager } = await detectDistro(clientId);
...
app.post('/api/packages/install', requireAuth, async (req, res) => {
    ...
    try {
        ...
        session.conn.exec(cmd, (err, stream) => {
            if (err) {
                sendLog(clientId, { type: 'package-install-progress', data: 'Error: ' + err.message, done: true, success: false });
                return;
            }
            stream.on('data', (d) => {
                sendLog(clientId, { type: 'package-install-progress', data: d.toString('utf-8') });
            });
            stream.stderr.on('data', (d) => {
                sendLog(clientId, { type: 'package-install-progress', data: d.toString('utf-8') });
            });
            stream.on('close', (code) => {
                sendLog(clientId, { type: 'package-install-progress', done: true, success: code === 0, packageName });
                logAudit('package_install', `Installed package: ${packageName} (exit: ${code})`, '', req);
            });
        });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/docker/install', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (['ubuntu', 'debian'].includes(distro)) {
            installCmd = 'apt-get update 2>&1 && DEBIAN_FRONTEND=noninteractive apt-get install -y docker.io docker-compose-plugin 2>&1; systemctl start docker; systemctl enable docker';
        } else if (distro === 'amzn') {
            installCmd = 'yum install -y docker 2>&1 && systemctl start docker && systemctl enable docker';
        } else {
        ...
        session.conn.exec(installCmd, (err, stream) => {
            if (err) {
                sendLog(clientId, { type: 'docker-install-progress', data: 'Error: ' + err.message });
                sendLog(clientId, { type: 'docker-install-complete', success: false });
                return;
            }
            stream.on('data', (d) => {
                sendLog(clientId, { type: 'docker-install-progress', data: d.toString('utf-8') });
            });
            stream.stderr.on('data', (d) => {
                sendLog(clientId, { type: 'docker-install-progress', data: d.toString('utf-8') });
            });
            stream.on('close', (code) => {
                sendLog(clientId, { type: 'docker-install-complete', success: code === 0 });
                logAudit('docker_install', 'Docker installation ' + (code === 0 ? 'completed' : 'failed'), '', req);
            });
        });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/security/status', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const openPorts = (sections[0] || '').trim().split('\n').filter(l => l.trim()).map(l => {
        ...
        if (firewallRaw.includes('Status: active') || firewallRaw === 'running') firewallStatus = 'active';
        else if (firewallRaw.includes('Status: inactive') || firewallRaw === 'not running') firewallStatus = 'inactive';
        else if (firewallRaw.includes('no-firewall')) firewallStatus = 'not-installed';
        ...
        res.json({
            success: true,
            security: { openPorts, firewallStatus, firewallRaw, fail2banInstalled, fail2banRaw, sshConfig, failedLogins }
        });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/security/fail2ban/install', requireAuth, async (req, res) => {
    ...
    try {
        ...
        session.conn.exec(cmd + ' 2>&1', (err, stream) => {
            if (err) return sendLog(clientId, { type: 'security-install-progress', done: true, success: false, data: err.message });
            stream.on('data', (d) => sendLog(clientId, { type: 'security-install-progress', data: d.toString('utf-8') }));
            stream.stderr.on('data', (d) => sendLog(clientId, { type: 'security-install-progress', data: d.toString('utf-8') }));
            stream.on('close', (code) => {
                sendLog(clientId, { type: 'security-install-progress', done: true, success: code === 0 });
                logAudit('fail2ban_install', 'Fail2ban installation ' + (code === 0 ? 'completed' : 'failed'), '', req);
            });
        });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/security/firewall/enable', requireAuth, async (req, res) => {
    ...
    try {
        ...
        let cmd;
        if (pkgManager === 'apt') {
            cmd = 'apt-get install -y ufw && ufw default deny incoming && ufw default allow outgoing && ufw allow 22/tcp && echo "y" | ufw enable';
        } else {
        ...
        session.conn.exec(cmd + ' 2>&1', (err, stream) => {
            ...
            stream.on('close', (code) => {
                sendLog(clientId, { type: 'security-install-progress', done: true, success: code === 0 });
                logAudit('firewall_enable', 'Firewall ' + (code === 0 ? 'enabled' : 'failed'), '', req);
            });
        });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/security/firewall/rule', requireAuth, async (req, res) => {
    ...
    if (!clientId || !port) return res.status(400).json({ error: 'clientId and port required' });
    const portNum = parseInt(port);
    if (isNaN(portNum) || portNum < 1 || portNum > 65535) return res.status(400).json({ error: 'Invalid port number' });
...
app.post('/api/security/ssh/port', requireAuth, async (req, res) => {
    ...
    if (!clientId || !port) return res.status(400).json({ error: 'clientId and port required' });
    const portNum = parseInt(port);
    if (isNaN(portNum) || portNum < 1024 || portNum > 65535) return res.status(400).json({ error: 'Port must be 1024-65535' });
    try {
        const cmd = [
...
app.post('/api/ssh-keys/generate', requireAuth, (req, res) => {
    try {
        ...
        const sshPub = require('crypto').createPublicKey(publicKey).export({ type: 'spki', format: 'der' });
...
app.post('/api/batch/start', requireAuth, async (req, res) => {
    const { hosts, username, password, pemContent, operation, clientId } = req.body;
    if (!hosts || !Array.isArray(hosts) || hosts.length === 0) return res.status(400).json({ error: 'hosts array required' });
    if (hosts.length > 20) return res.status(400).json({ error: 'Maximum 20 hosts per batch' });
    if (!password) return res.status(400).json({ error: 'password required' });

    const validOps = ['setup-password', 'install-docker', 'install-packages', 'security-harden'];
    if (!validOps.includes(operation)) return res.status(400).json({ error: 'Invalid operation' });

    const batchId = uuidv4();
    const results = {};
    hosts.forEach(h => { results[h] = { status: 'pending', logs: [], startedAt: null, endedAt: null }; });
    batchJobs.set(batchId, { hosts, results, startedAt: Date.now(), operation, cancelled: false });
    ...
    const processBatchHost = async (host) => {
        ...
        sendLog(clientId, { type: 'batch-host-progress', batchId, host, status: 'running' });
        ...
        return new Promise((resolve) => {
            const timeout = setTimeout(() => {
                ...
                sendLog(clientId, { type: 'batch-host-progress', batchId, host, status: 'failed', message: 'Timeout' });
                resolve();
            }, 30000);
            ...
            conn.on('ready', async () => {
                ...
                try {
                    ...
                    sendLog(clientId, { type: 'batch-host-progress', batchId, host, status: 'success' });
                } catch (err) {
                    results[host].status = 'failed';
                    results[host].logs.push(err.message.slice(0, 500));
                    sendLog(clientId, { type: 'batch-host-progress', batchId, host, status: 'failed', message: err.message.slice(0, 200) });
                } finally {
                    results[host].endedAt = Date.now();
                    conn.end();
                    resolve();
            ...
            conn.on('error', (err) => {
                ...
                results[host].endedAt = Date.now();
                sendLog(clientId, { type: 'batch-host-progress', batchId, host, status: 'failed', message: err.message });
                resolve();
            });

            const connectConfig = {
                host, port: 22, username: username || 'ec2-user',
    ...
    (async () => {
        ...
        logAudit('batch_' + operation, `Batch ${operation} on ${hosts.length} hosts`, '', req);
...
app.post('/api/ssh-connect', requireAuth, (req, res) => {
    ...
    if (!ec2Host) {
        return res.status(400).json({ success: false, error: 'EC2 Host is required' });
    }
    ...
    if (!wsClient || wsClient.readyState !== WebSocket.OPEN) {
        console.error(`WebSocket lookup failed for clientId: ${clientId}, known clients: [${Array.from(clients.keys()).join(', ')}]`);
        return res.status(400).json({ success: false, error: 'WebSocket session expired. Please refresh the page and try again.' });
    }

    console.log(`SSH connect requested: ${username || 'ec2-user'}@${ec2Host} [client: ${clientId}]`);

    const conn = new SSHClient();

    conn.on('ready', () => {
        console.log(`SSH authenticated: ${username || 'ec2-user'}@${ec2Host} [client: ${clientId}]`);

        // Update session inventory data
        try {
            db.prepare(`UPDATE sessions SET last_connected = datetime('now'), connection_result = 'success' WHERE host = ?`).run(ec2Host);
        } catch (_) {}
        ...
        conn.shell({ term: 'xterm-256color', cols: 120, rows: 30 }, (err, stream) => {
            ...
            stream.on('close', () => {
                console.log(`SSH stream closed: root@${ec2Host} [client: ${clientId}]`);
                terminals.delete(clientId);
                conn.end();
    ...
    conn.on('error', (err) => {
        ...
        try {
            db.prepare(`UPDATE sessions SET last_connected = datetime('now'), connection_result = 'failed' WHERE host = ?`).run(ec2Host);
        } catch (_) {}
    ...
    conn.on('close', () => {
        console.log(`SSH connection closed: ${username || 'ec2-user'}@${ec2Host} [client: ${clientId}]`);
        terminals.delete(clientId);
    });
    ...
    logAudit('ssh_connect', `SSH connect: ${username || 'ec2-user'}@${ec2Host}`, ec2Host, req);
...
function gracefulShutdown(signal) {
    ...
    for (const [id, session] of terminals) {
        ...
        try { if (session.stream) session.stream.destroy(); } catch (_) {}
    ...
    try {
        ...
        if (uploads.length) console.log(`  Cleaned ${uploads.length} uploaded file(s)`);
    } catch (_) {}
